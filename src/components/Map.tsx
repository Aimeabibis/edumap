...
import { Map, MapEvent, Marker, Popup } from "@vis.gl/react-maplibre";
...
export interface IReactMap {
  data?: ISchoolModel[];
  form?: any;
  objetKey?: {longitude: string; latitude: string};
  styleMap?: {
    height: number;
  };
}

export const ReactMap = ({ styleMap = { height: 400 }, data, form, objetKey }: IReactMap) => {
  ...
  const handleClick = (event: { lngLat: { lng: number; lat: number } }) => {
    ...
    if (form && objetKey) {
      form[objetKey.longitude] = lng; // Access the 'longitude' property
      form[objetKey.latitude] = lat; // Access the 'latitude' property
      form.setFieldValue("latitude", lat); // Ensure form has setFieldValue method
    }
  ...
  return (
    <>
          ...
          >
            <div className="flex flex-col">
              <button
                ...
                className="bg-green-700 mb-1 text-[16px] text-white font-bold py-2 px-4 rounded mt-2"
              >
                Faire un don
              </button>
      ...
      {isOpen && (
        <Elements
          stripe={loadStripe(
            "pk_test_51RLSDlISvHt6ormbhQ6fSipchkCeoaHiA6BEUTPwhKa42UIWWFE7P9WSGzVM0qvM4eG5VHqQQZQRn2YIe6a9kK4q00J4bqqxG4"
          )}
        >
